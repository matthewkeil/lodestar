...
export function getValidatorApi({
  ...
  async function engineOnlyBlockProductionFlow({
    feeRecipient,
    slot,
    randaoReveal,
    graffiti,
  ...
  async function builderOnlyBlockProductionFlow({
    feeRecipient,
    slot,
    randaoReveal,
    graffiti,
    ...
    routes.validator.ProduceBlindedBlockOrContentsRes & {
  ...
  > {
    ...
    try {
      const block = await produceBlindedBlockOrContents(slot, randaoReveal, graffiti, {
        feeRecipient,
        // skip checking and recomputing head in these individual produce calls
        skipHeadChecksAndUpdate: true,
      });
  ...
  async function builderAndExecutionBlockProductionFlow({
    feeRecipient,
    slot,
    randaoReveal,
    graffiti,
    ...
    if (builder.status === "fulfilled" && engine.status === "pending") {
      logger.verbose("Builder won the race", {durationMs: Date.now() - startTime, slot});

      return {...builder.value, executionPayloadBlinded: true} as routes.validator.ProduceBlindedBlockOrContentsRes & {
        executionPayloadBlinded: true;
      };
    }

    if (builder.status === "pending" && engine.status === "fulfilled") {
    ...
    if (builder.status === "fulfilled" && engine.status === "fulfilled") {
      ...
      if (selectedSource === ProducedBlockSource.engine) {
        ...
        return {...builderBlock, executionPayloadBlinded: true} as routes.validator.ProduceBlindedBlockOrContentsRes & {
          executionPayloadBlinded: true;
        };
      }
    }
  ...
  const produceBlockV3: ServerApi<routes.validator.Api>["produceBlockV3"] = async function produceBlockV3(
    ...
    if (isBuilderEnabled && builderSelection === routes.validator.BuilderSelection.BuilderOnly) {
      return builderOnlyBlockProductionFlow({feeRecipient, graffiti, randaoReveal, slot});
    }

    // If builder is disabled ignore builder selection of builderonly if caused by user mistake
    if (!isBuilderEnabled) {
      return engineOnlyBlockProductionFlow({feeRecipient, graffiti, randaoReveal, slot, strictFeeRecipientCheck});
    }

    return builderAndExecutionBlockProductionFlow({
      feeRecipient,
      graffiti,
      randaoReveal,
      slot,
